...
export class Sunservice {
	...
	public setFeatures(time: Date): void {
		...
		const newFeatures = angles.map((angle) => {
			...
			const polygon = this.createCircularPolygon(center, radius);

			return new Feature<MultiPolygon>(polygon.transform("EPSG:4326", "EPSG:3857"));
		});
	...
	private createCircularPolygon(center: number[], radius: number): MultiPolygon {
	...
	private getShadowRadiusFromAngle(angle: number): number {
	...
	private getShadowPosition(time: Date): number[] {
		const sunPosition = this.calculateSunPosition(time);

		return [sunPosition[0] + 180, -sunPosition[1]];
	}

	private calculateSunPosition(time: Date): number[] {
